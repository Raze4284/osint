...
class SearchModule(loader.Module):
    ...
    async def process_command(self, message, bot_username):
        ...
        if len(args) > 1:
            query_value = args[1]
            status_message = await message.reply(f"📱 Пожалуйста, отправьте следующее значение боту {bot_username}:\n\n{query_value}\n\nОжидаю ответ от бота...")
    ...
    async def osphone_cmd(self, message):
        ...
        await self.process_command(message, "@FS00CIETY_bot")  # Исправлена кавычка
